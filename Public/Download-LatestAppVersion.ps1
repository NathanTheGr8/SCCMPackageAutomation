        ...
        try {
            switch ($App) {
                ...
                'putty' {
                    ...
                    $links = (Invoke-WebRequest -uri $url -UseBasicParsing).links
                    $downloadLinks = $links | Where-Object -Property href -like "*putty-*$newAppVersion-installer.msi"

                    # Select 64 and 32bit download links
                    $32bitDownload = $downloadLinks.href -notlike "*64bit*" | Select-Object -First 1
                    $64bitdownload = $downloadLinks.href -like "*64bit*" | Select-Object -First 1
            ...
            Write-Error "Download of $app failed"
            Write-Error "$_"
        }
