function New-ApplicationHelper {
    ...
    try {
        $DeploymentTypeOutput = Add-CMScriptDeploymentType -ApplicationName "$AppNameFull" -DeploymentTypeName "Script $AppNameFull" `
            ...
            -ScriptLanguage PowerShell -ScriptFile "$appPath\SupportFiles\SCCMApplicationDetection.ps1" -CacheContent -ContentFallback `
            -ContentLocation "$appPath" -EstimatedRuntimeMins "$EstimatedRuntimeMins" -MaximumRuntimeMins "$MaximumRuntimeMins" -InstallationBehaviorType "InstallForSystem" `
